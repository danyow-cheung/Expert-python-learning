...
##### objgraph 
objgraph（請參閱 http://mg.pov.lt/objgraph/）是一個用於創建對象引用圖的簡單工具，在 Python 中尋找內存洩漏時應該很有用。

objgraph 提供了多個實用程序，允許您列出和打印有關內存使用情況和對象計數的各種統計信息。 下面的解釋器會話記錄中顯示了正在使用的此類實用程序的示例。
```python
import objgraph
objgraph.show_most_common_types()
```
如前所述，objgraph 允許您創建內存使用模式圖和鏈接給定命名空間中所有對象的交叉引用。 該庫最有用的圖表工具是 objgraph.show_refs() 和 objgraph.show_backrefs()。 它們都接受對被檢查對象的引用，並使用 Graphviz 包將圖表圖像保存到文件中。 圖 2 和圖 3 顯示了此類圖表的示例。
以下是用於創建這些圖表的代碼：
> chapter11/graphviz.py


當這樣一個循環中的至少一個對象定義了自定義 __del__() 方法時，真正的問題就開始了。 它是一個自定義的釋放處理程序，當對象的引用計數最終變為零時將被調用。 它可以執行任意 Python 代碼，因此還可以創建對特色對象的新引用。 這就是為什麼如果至少有一個對象提供自定義 __del__() 方法實現，則 Python 3.4 版本之前的垃圾收集器無法中斷引用循環的原因。 PEP 442 向 Python 引入了安全對象終結，並從 Python 3.4 開始成為標準的一部分。 無論如何，對於擔心向後兼容性並針對廣泛的 Python 解釋器版本的包來說，這可能仍然是一個問題。 以下代碼片段向您展示了循環垃圾收集器在不同 Python 版本中的行為差異：
> chapter11/garbage_collector.py 
#### C code memory leaks
如果 Python 代碼看起來非常好，並且當您循環遍歷隔離函數時內存仍然增加，則洩漏可能位於 C 端。 例如，當缺少 Py_DECREF 調用時會發生這種情況。

Python 核心代碼非常健壯並且經過了洩漏測試。 如果您使用具有 C 擴展的包，那麼它們可能是首先查看的好地方。 因為你
有 C 擴展，它們可能是一個很好的地方。 因為您將處理在比 Python 低得多的抽象級別上運行的代碼，所以您需要使用完全不同的工具來解決此類內存問題。

C 中的內存調試並不容易，因此在深入研究擴展內部之前，請確保正確診斷問題的根源。 這是一種非常流行的方法，可以用本質上類似於單元測試的代碼來隔離可疑包：
為您懷疑內存洩漏的每個 API 單元或擴展功能編寫單獨的測試
隔離地在任意長的時間內循環執行測試（每次運行一個測試）
從外部觀察哪些被測試的功能會隨著時間的推移增加內存使用
使用這種方法，您可以隔離擴展的錯誤部分，這將減少以後檢查和修復其代碼所需的時間。 這個過程可能看起來很繁瑣，因為它需要大量額外的時間和編碼，但從長遠來看確實值得。 您始終可以通過重用第 10 章“測試驅動開發”中介紹的一些測試工具來簡化您的工作。 諸如 tox 之類的實用程序可能並不是專門為這種情況設計的，但它們至少可以減少在隔離環境中運行多個測試所需的時間。


### Profiling network usage
正如我之前所說，與第三方程序（如數據庫、緩存、Web 服務或 LDAP 服務器）通信的應用程序在運行緩慢時可能會變慢。 這可以在應用程序端使用常規代碼分析方法進行跟踪。 但如果第三方軟件本身運行良好，罪魁禍首可能是網絡。
問題可能是錯誤配置的集線器、低帶寬網絡鏈接，甚至是導致計算機多次發送相同數據包的大量流量衝突。
這裡有一些讓您了解的要素。要了解發生了什麼，首先要調查三個領域：
- watch the network traffic using tools 
  - ntop
  - wireshark
- track down unhealthy or misconfigured devices with net-snmp 
- Estimate the bandwidth between two computers using Pathrate,a statistical tool 
