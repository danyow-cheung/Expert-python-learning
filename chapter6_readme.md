...
- TLS/SSL
- 非特權用戶不能綁定低端口（0-1000範圍內），但HTTP協議應該在80端口為用戶服務，HTTPS應該在443端口服務。
- 通常，Nginx 可以比 Python 代碼更高效地服務於靜態資產（圖像、JS、CSS 和其他媒體）。
- 當單個主機需要服務於來自不同域的多個應用程序時，Apache 或 Nginx 是不可或缺的，它們可以為同一端口上服務的不同域創建虛擬主機。
- 反向代理可以通過添加額外的緩存層來提高性能，或者可以配置為簡單的負載平衡器。

某些 Web 服務器實際上建議在代理後面運行，例如 Nginx。 
例如，gunicorn 是一個非常健壯的基於 WSGI 的服務器，如果它的客戶端也很快，
它可以提供出色的性能結果。 另一方面，它不能很好地處理慢速客戶端，
因此很容易受到基於慢速客戶端連接的拒絕服務攻擊。 使用能夠緩衝慢速客戶端的代理服務器是解決此問題的最佳方法。

### Reloading process gracefully 
十二因素應用程序方法論的第九條規則處理過程可處置性，並指出您應該通過快速啟動時間和優雅關閉來最大限度地提高穩健性。 雖然快速啟動時間是不言自明的，但優雅的關閉需要一些額外的討論。

## Code instrumentation and monitoring 代碼檢測和監控
我們的工作不會以編寫應用程序並將其部署到目標執行環境而結束。 編寫部署後不需要任何進一步維護的應用程序是可能的，儘管這種可能性很小。 實際上，我們需要確保正確觀察錯誤和性能。
為確保我們的產品按預期工作，我們需要正確處理應用程序日誌並監控必要的應用程序指標。 這通常包括：
- 監控各種 HTTP 狀態代碼的 Web 應用程序訪問日誌
- 可能包含有關運行時錯誤和各種警告的信息的進程日誌集合
- 監控運行應用程序的遠程主機上的系統資源（CPU 負載、內存和網絡流量）
- 監控作為業務績效指標（客戶獲取、收入等）的應用程序級性能和指標

幸運的是，有許多免費工具可用於檢測您的代碼並監控其性能。 它們中的大多數都非常容易集成。

### Logging errors - sentry/raven 
您可以做的是為此類情況做好充分準備，並確保沒有任何錯誤被忽視。 
在大多數情況下，任何意外的故障場景都會導致應用程序引發異常並通過日誌系統記錄下來。 
這可以是 stdout、sderr、文件或您為日誌記錄配置的任何輸出。 
根據您的實施，這可能會或可能不會導致應用程序以某些系統退出代碼退出。

正確的日誌處理對於其他原因非常重要，稍後將解釋這些原因，但不能很好地跟踪和調試生產錯誤。

您真正需要的是盡可能多的有關錯誤發生的上下文信息。 擁有生產環境中發生的錯誤的完整歷史記錄也非常有用，
您可以通過某種方便的方式瀏覽和搜索這些錯誤。 提供此類功能的最常見工具之一是 **Sentry** (https://getsentry.com)。 
它是一項久經考驗的服務，用於跟踪異常和收集崩潰報告。 它是開源的，是用 Python 編寫的，起源於後端 Web 開發人員的工具。 
現在，它已經超越了最初的雄心壯志，支持更多語言，包括 PHP、Ruby 和 JavaScript，但仍然是大多數 Python Web 開發人員選擇的最受歡迎的工具。
### Monitoring system and application metrics 
Munin (http://munin-monitoring.org) 是許多組織使用的流行選擇之一，無論他們使用何種技術堆棧。

它的安裝包括兩個主要組件：
- 從其他節點收集指標並提供指標圖的 Munin master
- 安裝在受監控主機上的 Munin 節點，它收集本地指標並將其發送給 Munin master

### Dealing with application logs 
請記住，日誌不僅與錯誤有關。 許多開發人員過去只將日誌視為在調試問題時有用和/或可用於執行某種取證的數據源。 
當然，很少有人嘗試將其用作生成應用程序指標或進行一些統計分析的來源。 但日誌可能比這有用得多。 它們甚至可以成為產品實施的核心。
#### Basic low-level log practices 
處理日誌的最佳約定可以總結為以下幾條規則：
- 應用程序應始終將未緩衝的日誌寫入標準輸出 (stdout)
- 執行環境應該負責日誌的收集和路由到最終目的地

#### Tools for log processing 
如果您沒有處理大量日誌的經驗，那麼在使用具有一定負載的產品時，您最終會獲得這種經驗。 您很快就會注意到，基於將它們存儲在文件中並在某些持久存儲中備份以供以後檢索的簡單方法是不夠的。 如果沒有合適的工具，這將變得粗糙且昂貴。 諸如 logrotate 之類的簡單實用程序只能幫助您確保硬盤不會被不斷增加的新事件溢出，但是拆分和壓縮日誌文件僅有助於數據歸檔過程，而不會使數據檢索或分析變得更簡單。
